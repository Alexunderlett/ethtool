...
func main() {
	...
	fmt.Println("length: ",len(data.Messages))
	...
	contractAddress := common.HexToAddress("0xCFC1C71a7673644537e22236356f792a1B725577") //合约地址
	sourceAddress := common.HexToAddress("0xfCEAdAFab14d46e20144F48824d0C09B1a03F2BC") //来源地址
	...
	number,err := inst.SetCompleted(txOpts,big.NewInt(6_000_000_000))
	assert(err)
	...
	for i:=0; i<length; i++ {
		...
		intValue, _ := strconv.Atoi(data.Prices[strCoin[i]])
		newValue := int(intValue)
		fmt.Println(newValue)
		rep,err := inst.Puts(txOpts,sourceAddress,timestamps,strCoin[i],newValue)
		assert(err)
		fmt.Println("rep: ", rep.Hash())
	}
